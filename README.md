...
- При старте с открытыми позициями активной становится позиция с минимальной ценой продажи — новая покупка не делается
...
- Сбои Telegram API не убивают торговый алгоритм (`_safe_send` обёртка)
- Одно уведомление о проблемах с WebSocket за outage (без флуда)
- Автостоп только после 20 подряд неожиданных ошибок (не на первой)
- Еженедельный авто-рестарт через systemd timer (страховка от утечки памяти)
